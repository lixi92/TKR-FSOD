...
class GeneralizedRCNN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.MODEL.TRAIN_GCN:
            for p in self.roi_heads.box_predictor.parameters():
                p.requires_grad = False
            for p in self.roi_heads.pooler.parameters():
                p.requires_grad = False
            for p in self.roi_heads.reduce_channel.parameters():
                p.requires_grad = False
            for p in self.roi_heads.bbox_roi_extractor.parameters():
                p.requires_grad = False
            for p in self.roi_heads.deform_box_predictor.parameters():
                p.requires_grad = False
            for p in self.roi_heads.fc_s.parameters():
                p.requires_grad = False
            for p in self.affine_rpn.parameters():
                p.requires_grad = False
            for p in self.affine_rcnn.parameters():
                p.requires_grad = False
