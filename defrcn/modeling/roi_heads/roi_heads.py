...
from ..gcn.gmm_conv import GMMConv
import torch.nn.functional as F
from ..deform_pool.single_level import SingleRoIExtractor
...
class Res5ROIHeads(ROIHeads):
    ...
    def __init__(self, cfg, input_shape): 
        ...
        #------------------------------------------- 新加的 --------------------------------#
        self.deform_weight = cfg.MODEL.DEFORM_WEIGHT
        self.deform_output_channels = 256
        self.reduce_channel = nn.Conv2d(in_channels=1024, out_channels=self.deform_output_channels, kernel_size=1, stride=1, padding=0)
        self.bbox_roi_extractor = SingleRoIExtractor(
            out_channels=self.deform_output_channels,
            featmap_strides=[4,]
        )
        weight_init.c2_msra_fill(self.reduce_channel)
        self.fc_s = nn.Linear(out_channels + self.deform_output_channels, out_channels)
        for layer in [self.fc_s]:
            weight_init.c2_xavier_fill(layer)

        self.deform_box_predictor = DoubleFastRCNNOutputLayers(
            cfg, out_channels , self.num_classes, self.cls_agnostic_bbox_reg
        )
    ...
    def _shared_roi_transform(self, features, boxes):
        x = self.pooler(features, boxes) # batch_size=4时，features[0].shape: [4, 1024, 46, 68], x的shape为[2048, 1024, 7, 7]
        # print('pooler:', x.size())
        x = self.res5(x) # 经过res5后的x.shape: [2048, 2048, 4,4]
        # print('res5:', x.size())
        return x
    ...
    def get_pesudo_system(self, relation, bbox_pred_ctr, device):
        ...
        d = torch.sqrt((coord_i[:, 0] - coord_j[:, 0]) ** 2 + (coord_i[:, 1] - coord_j[:, 1]) ** 2) # [num_roi_per_image * relation_k]
    ...
    def _enhance_feature(self, num_images, box_features, feature_pooled, pred_class_logits,
        ...
        indices = torch.zeros(k, num_rois_per_image * num_images)
        for i in range(1, num_images + 1):
            sub_z = z[num_rois_per_image * (i - 1) : num_rois_per_image * i]
            sub_eps = torch.mm(sub_z, sub_z.t())
            _, sub_indices = torch.topk(sub_eps, k=k, dim=0) # indice shape: [32, 512] 对于每个roi 取最相关的32条边的索引
            indices[:, num_rois_per_image * (i - 1) : num_rois_per_image * i] = sub_indices + num_rois_per_image * (i - 1)    
        ...
        bbox_pred_ctr = bbox_pred_cls[:, 0:2] + bbox_pred_cls[:, 2:4] # shape: [512 * batch_size, 2] 每个roi的center坐标

        relation = torch.empty(2, self.gcn_relation_k * num_rois, dtype=torch.long).to(device) # shape: [2, 32 * 512 * batch_size = 16384] 所有的边
        # U = torch.empty(32*128, 2).to(self._device)

        # 同一条边的两个节点索引
        relation[0] = torch.Tensor(list(range(num_rois)) * self.gcn_relation_k)  # , type=torch.long)
        relation[1] = indices.view(-1) # shape: [16384] indices按行连接

        U = self.get_pesudo_system(relation, bbox_pred_ctr, device)
    ...
    def forward(self, images, features, proposals, targets=None):
        ...
        proposal_boxes = [x.proposal_boxes for x in proposals] # list中每个bbox的tensor shape 为[512, 4]
        backbone_output_features = [features[f] for f in self.in_features]
        box_features = self._shared_roi_transform(
            backbone_output_features, proposal_boxes
        ) # batch_size = 4时，box_features.shape为[2048, 2048, 4,4] backbone output的shape为[[4, 1024, 40, 59]]
        ...
        # 使用deform_pool得到的特征
        deform_features = self.reduce_channel(backbone_output_features[0])
        feature_deform_pooled = self.bbox_roi_extractor([deform_features], proposal_boxes) # batch_size=4时，feature_deform_pooled.shape为[2048, 1024, 7, 7]
        feature_deform_pooled = feature_deform_pooled.mean(dim=[2, 3])  # pooled to 1x1
        
        feature_pooled_s = torch.cat([feature_pooled, feature_deform_pooled], dim=1)

        feature_pooled_s = F.relu(self.fc_s(feature_pooled_s))

        deform_pred_class_logits = self.deform_box_predictor(feature_pooled_s)
        
        
        if self.train_gcn:
            pred_class_logits, pred_proposal_deltas = self._enhance_feature(
                num_images, box_features, feature_pooled, pred_class_logits, pred_proposal_deltas)
            pred_class_logits = pred_class_logits * (1 - self.deform_weight) + deform_pred_class_logits * self.deform_weight
        else:
            pred_class_logits = pred_class_logits * (1 - self.deform_weight) + deform_pred_class_logits * self.deform_weight
