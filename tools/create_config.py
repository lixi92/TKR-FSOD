...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='coco14', help='', choices=['coco14', 'voc', 'qiunao'])
...
def main():
    ...
    if args.dataset in ['voc']:
        ...
        for i, lineinfo in enumerate(yaml_info):
            if '  TRAIN: ' in lineinfo:
                _str_ = '  TRAIN: ("{}_trainval_{}{}_{}shot_seed{}", )\n'
                if 'gcn' in lineinfo:
                    _str_ = '  TRAIN: ("{}_gcn_trainval_{}{}_{}shot_seed{}", )\n'
                yaml_info[i] = _str_.format(args.dataset, suffix, args.split, args.shot, args.seed)
            if '  TEST: ' in lineinfo:
                _str_ = '  TEST: ("{}_test_{}{}",)\n'
                yaml_info[i] = _str_.format(args.dataset, suffix, args.split)
        yaml_path = yaml_path.replace('novelx', 'novel{}'.format(args.split))
    else:
